...
function updateBalanceDue(currencyPicker) {
    ...
    for(let i=0; i < quantity.length; i++) {
        ...
        if(!isNaN(Number((Number(quantity[i].value)*Number(rate[i].value)).toFixed(2))))
            amount[i].innerHTML=currencyPicker+' '+Number((Number(quantity[i].value)*Number(rate[i].value)).toFixed(2));
...
function updateDue(sum,currencyPicker) {
    ...
    if(!isNaN(Number((sum-paid).toFixed(2))))
        document.querySelector('#balanceDue').innerHTML=currencyPicker+' '+Number((sum-paid).toFixed(2));
    if(!isNaN(Number(sum.toFixed(2))))
        document.querySelector('#totalAmount').innerHTML=currencyPicker+' '+Number(sum.toFixed(2));
}
...
function subtotal(sum) {
    if(!isNaN(Number(sum.toFixed(2)))) {
        ...
        document.querySelector('#subtotalPrice').innerHTML=currencyPicker+' '+Number(sum.toFixed(2));
    }
}
...
function updateAll() {
    ...
    updateDue(sum-discount+shipping+Number(document.querySelector('#taxPercent').value)/100*(sum-discount),currencyPicker);
    return sum;
}